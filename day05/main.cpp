...
void part2()
{
    ...
    for(int i = 0; i < lines.size(); i++)
    {
        ...
        else if((std::abs(lines[i].x1 - lines[i].x2) == std::abs(lines[i].y1 - lines[i].y2)))
        {
            if(lines[i].x1 > lines[i].x2 && lines[i].y1 > lines[i].y2)
            {
                int currentX = lines[i].x2;
                for(int yiter = lines[i].y2; yiter <= lines[i].y1; yiter++)
                {
                    ventsMap[yiter][currentX] += 1;
                    currentX++;
                }
            }
            else if(lines[i].x2 > lines[i].x1 && lines[i].y2 > lines[i].y1)
            {
                int currentX = lines[i].x1;
                for(int yiter = lines[i].y1; yiter <= lines[i].y2; yiter++)
                {
                    ventsMap[yiter][currentX] += 1;
                    currentX++;
                }
            }
            else if(lines[i].x2 > lines[i].x1 && lines[i].y1 > lines[i].y2)
            {
                int currentX = lines[i].x2;
                for(int yiter = lines[i].y2; yiter <= lines[i].y1; yiter++)
                {
            ...
            else if(lines[i].x1 > lines[i].x2 && lines[i].y2 > lines[i].y1)
            {
                int currentX = lines[i].x1;
                for(int yiter = lines[i].y1; yiter <= lines[i].y2; yiter++)
                {
